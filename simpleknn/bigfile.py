...
class BigFile:
    ...
    def __init__(self, datadir):
        ...
        self.names = open(id_file, encoding='ISO-8859-1').read().encode('utf-8').strip().split() # Bytes
        self.names = [x.decode() for x in self.names] # str
    ...
    def read(self, requested, isname=True):
        ...
        #print("word : ", type(requested[0]))
        #print("vocab : ", type(list(self.name2index.keys())[0]))
        if isname:
            #print(requested[0] in self.name2index)
            index_name_array = [(self.name2index[x], x) for x in requested if x in self.name2index]
            #print(index_name_array)
        else:
