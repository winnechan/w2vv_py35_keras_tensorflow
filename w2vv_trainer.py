import os
os.environ["PATH"] += os.pathsep + 'E:\\code\\w2vv-master\\graphviz-2.38\\release\\bin'
...
def process(opt, trainCollection, valCollection, testCollection):
    ...
    opt.n_text_layers = list(map(int, opt.n_text_layers.strip().split('-')))
    ...
    checkpoint_dir = os.path.join(rootpath, trainCollection, opt.checkpoint + '_w2vv_' + valCollection \
                 + "_" + opt.text_style + '_' + text_name + "_" + opt.img_feature + "_" + model_style + "_" + opt.postfix)
    ...
    if opt.model_name.endswith('_ms'):
        ...
        #print("word embedding shape ", we_weights.shape)
        #sys.exit()
    ...
    #for epoch in range(1):
        ...
        #for minibatch_index in xrange(10):
            n_step += 1
            img_X_batch, text_X_batch = trainBatchIter.__next__()
